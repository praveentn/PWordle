...
def pick_random_word():
    ...
    r = RandomWords()
    true_word = r.get_random_word(hasDictionaryDef="true", includePartOfSpeech="noun,verb,adjective", minDictionaryCount = 3, minLength=minlen, maxLength=minlen)
...
def ask_user_input(guru = None):
    while True:
        guru = input ("Enter a " +str(minlen) +"-letter Word :")
        if type(guru) != str or len(guru) != minlen:
            print('Invalid Entry')
            continue
        else:
            return guru.upper()
...
def build_df(attempt, guess_arr):
    # cols = ['L1', 'L2', 'L3', 'L4', 'L5']
    cols = ['L' + str(i) for i in range(1, minlen+1)]
...
def play_wordle(n):
    ...
    while attempt < 6:
        ...
        if guess_word.lower() == true_word:
            ...
            print('##############################################################')
            print('YOU HAVE FOUND YOUR MATCH!!! DON\'T FORGET TO SAY HI TO HIM!!!')
            print('##############################################################')
            print('')
